...
def addListSlug(user, drop, created_by):        
    if not user:
        return drop    
    if "lists" in list(user.keys()):        
        for l in user["lists"]:
            for m in l["members"]:                
                if str(created_by)==str(m):
                    if "list_slugs" in drop:
                        drop["list_slugs"].append(l["slug"])
                    else:
                        drop["list_slugs"] = [l["slug"]]
                    drop["list_slugs"] = list(set(drop["list_slugs"]))
    return drop
...
def saveTweets(tweet):
    ...
    drops = getCollDrops()
    if drops.count()==0:
        clog("MAKE INDEXES FIRST!!!")
        #exit(1)
        #drops.create_index([("id_str", 1)], unique=True)
        indexes = """
db.lists.ensureIndex({added_at_precise:1})

db.drops.ensureIndex({id_str:1})
db.drops.ensureIndex({id_str:1}, {unique: true, dropDups : true});
db.drops.ensureIndex({newsrivr_userid_md5:-1})
db.drops.ensureIndex({newsrivr_userid_md5:1, added_at_precise:-1})
db.drops.ensureIndex({created_by:1, newsrivr_userid_md5:1, added_at_precise:-1})
db.drops.ensureIndex({newsrivr_userid_md5:1, screen_name:1, added_at_precise:-1})
db.drops.ensureIndex({added_at_precise:-1})
db.drops.ensureIndex({created_at:-1})
db.drops.ensureIndex({list_slugs:1, added_at_precise:-1})

db.drops.ensureIndex({mentions:1, added_at_precise:-1})                          
db.drops.ensureIndex({favorites:1, added_at_precise:-1})
db.drops.ensureIndex({htmlmd5:1, newsrivr_userid_md5:1})
db.drops.ensureIndex({md5:1, newsrivr_userid_md5:1})
db.drops.ensureIndex({user_id_str:1})
db.drops.ensureIndex({favorites:1})        
db.drops.ensureIndex({user_id_str:1, created_at:-1})
db.drops.ensureIndex({retweet_id_str:1})
db.drops.ensureIndex({screen_name:1, retweeted:1})

db.users.ensureIndex({id_str:1}, {unique: true});
db.users.ensureIndex({screen_name:1})
db.users.ensureIndex({twitter_friend_list:1});

db.imagemd5.ensureIndex({id_str:1})
db.imagemd5.ensureIndex({tweet_id_str:1, histogram_md5:1}, {unique: true});

db.youtubetags.ensureIndex({id_str:1})
db.youtubetags.ensureIndex({tweet_id_str:1, videotag:1}, {unique: true});

db.stream.ensureIndex({created_at:1})
db.tweets_processed.ensureIndex({id_str:1})
db.tweets.ensureIndex({id_str:1})
        """
        clog(indexes)
    for userid in tweet["newsrivr_userid_md5"]:
        dropscoll = drops.find({"id_str":tweet["id_str"]})
        user = getCollUsers().find_one({"id_str":str(tweet["for_user"])})
        if not user:
            clog("no user found returning undone")
            return
        if dropscoll.count()==0:
            #print tweet["id_str"],tweet["user"]["name"],":", tweet["text"]
            txt = tweet["text"]
            if "retweeted_status" in tweet:
                txt = tweet["retweeted_status"]["text"]            
            drop = procesContentToNewsRivrDrop(txt, tweet["id_str"], tweet["user"]["screen_name"], userid, tweet["user"]["id_str"])
            if "retweeted_status" in tweet:
                drop["retweeted"] = True
                drop["notretweeted"] = False
                drop["retweet_name"] = tweet["retweeted_status"]["user"]["name"]
                drop["retweet_screen_name"] = tweet["retweeted_status"]["user"]["screen_name"]
                drop["retweet_count_available"] = False
                if "retweet_count" in tweet["retweeted_status"]:
                    retweet_count_available = False
                    if "+" in str(tweet["retweeted_status"]["retweet_count"]):
                        retweet_count_available = True
                    else:
                        if int(tweet["retweeted_status"]["retweet_count"])>1:
                            retweet_count_available = True
                    if retweet_count_available:
                        drop["retweet_count_available"] = True
                        drop["retweet_count"] = tweet["retweeted_status"]["retweet_count"]
                drop["retweet_profile_image_url"] = tweet["retweeted_status"]["user"]["profile_image_url"]
                drop["retweet_created_at"] = parse(tweet["retweeted_status"]["created_at"])
                drop["retweet_id_str"] = tweet["retweeted_status"]["id_str"]
            else:
                drop["retweeted"] = False
                drop["notretweeted"] = True
            drop["profile_image_url"]=tweet["user"]["profile_image_url"]
            drop["id_str"]=tweet["id_str"]
            drop["sequence"]=1000000000000
            drop["created_at"]=tweet["created_at"]
            drop["created_by"]=tweet["user"]["id_str"]
            drop["screen_name"]=tweet["user"]["screen_name"]
            drop["screen_name_lower"]=tweet["user"]["screen_name"].lower()
            drop["user_id_str"]=tweet["user"]["id_str"]
            drop["name"]=tweet["user"]["name"]
            drop["user"]=tweet["user"]
            
            #list stuff
            drop = addListSlug(user, drop, drop["created_by"])
            
            drop = base64ProfilePic(tweet, drop)                        
            newdrop = copy.deepcopy(drop)
            newdrop["newsrivr_userid_md5"]=[userid]
            if "user" in tweet:
                for u in getCollUsers().find({"twitter_friend_list":int(tweet["user"]["id_str"])}):
                    newdrop["newsrivr_userid_md5"].append(u["newsrivr_userid_md5"])
            newdrop["newsrivr_userid_md5"] = list(set(newdrop["newsrivr_userid_md5"]))
            newdrop["hidehtml"]=False
            dtn, dtnms = getTimeWithMS()
            newdrop["added_at"] = dtn
            newdrop["added_at_precise"] = dtnms
            if drops.find({"htmlmd5":newdrop["htmlmd5"], "newsrivr_userid_md5":userid}).count()!=0:
                newdrop["hidehtml"]=True
            if drops.find({"md5":newdrop["md5"], "newsrivr_userid_md5":userid}).count()==0:
                if drops.find({"id_str":newdrop["id_str"]}).count()==0:
                    try:
                        drops.insert(newdrop, safe=True)
                    except (pymongo.errors.DuplicateKeyError) as e:
                        pass 
                    except (Exception) as e:
                        try:
                            if "followed_links" in newdrop:
                                for l in newdrop["followed_links"]:
                                    if "image" in l:                        
                                        if "srcbase64" in l["image"]:
                                            del l["image"]["srcbase64"]
                            drops.insert(newdrop, safe=True)
                        except (Exception) as e:
                            mailException(e)
            retval = True
        else:
            clog("adding user to a drop")
            drop = drops.find_one({"id_str":tweet["id_str"]})
            drop["newsrivr_userid_md5"].append(userid)
            drop["newsrivr_userid_md5"] = list(set(drop["newsrivr_userid_md5"]))
            drop = addListSlug(user, drop, drop["created_by"])
            """
            if dropscollusers.count()==0:
                drop_from_somebody_else = dropscoll[0]
                copied_drop = copy.deepcopy(drop_from_somebody_else)
                del copied_drop["_id"] 
                copied_drop["newsrivr_userid_md5"]=userid
                if drops.find({"md5":copied_drop["md5"], "newsrivr_userid_md5":userid}).count()==0:
                    clog("using copied drop not getting it again")
                    drops.insert(copied_drop, safe=True)                   
                retval =  True
            """
    clog("done")
    return retval
...
def processTweet(tweet, queue):
    try:
        saveTweets(tweet)
    except (Exception) as e:
        mailException(e)
        mailStringToAdmin("tweet and error", str(tweet)+"<hr>"+str(e))
    queue.put(1)
    
def processTweets():
    multiprocess = True
    # reset crashed tweets
    tweets = getCollUnprocessedTweets()
    l = []
    cnt = 0
    queue = Queue()
    while True:
        ...
        if t:
            try:
                getCollUnprocessedTweets().remove({"id_str":t["id_str"]}, safe=True)
            except:
                print(t)
                traceback.print_exc(file=sys.stdout)                
            clog("processing tweets to drops")            
            cnt += 1        
            if cnt%5==0:
                clog(cnt)
            if multiprocess:
                p = Process(target=processTweet, args=(t, queue))                
                p.start()
                l.append(1)
            else:
                processTweet(t, queue)
        else:
            clog("processTweets: t = tweets.find_one() returned None!")
            return
        maxserver = 20
...
if __name__=="__main__":
    clog("tweets to drops daemon online")            
